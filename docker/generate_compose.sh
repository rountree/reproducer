...
if [ "$NODE_COUNT" -gt 1 ]; then
    for i in $(seq 2 $NODE_COUNT); do
        ...
        sed -n '/# NODE_TEMPLATE_START/,/# NODE_TEMPLATE_END/p' "$TEMPLATE" | \
            sed -e "s/node-N/node-${i}/g" \
                -e "s/flux-var-N/flux-var-${i}/g" \
                -e "s/NODECOUNT/${NODE_COUNT}/g" | \
...
# Add volumes section
echo "volumes:" >> "$OUTPUT"
echo "  flux-etc:" >> "$OUTPUT"
for i in $(seq 1 $NODE_COUNT); do
    echo "  flux-var-${i}:" >> "$OUTPUT"
done

echo "Generated $OUTPUT with ${NODE_COUNT} nodes (multi-container flux cluster)"
